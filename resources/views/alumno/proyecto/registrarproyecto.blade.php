...
<div class="card">
    ...
    <div class="card-body">
        ...
        @endif
        @if(Session::has('errorHasProject'))
        <div class="alert alert-danger" id="alert-fade">{{Session::get('error')}}</div>
        @endif
        ...
        {{-- <div class="container"> --}}
        <h6 class="card-title">{{$foro->no_foro}}º {{$foro->nombre}} <br>
        ...
        <form method="post" action="{{route('registrarProyecto',Crypt::encrypt($foro->id))}}" class="form-center">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control uppercase" type="text" name="titulo" placeholder=" Titulo" />
            ...
            <div class="row">
                <div class="col-lg-6 form-group">
                    ...
                    <select name="lineadeinvestigacion_id" id="categorias" class="form-control">
                        ...
                        <option value="{{Crypt::encrypt($linea->id)}}">{{$linea->nombre}}</option>
                        @endforeach
                    </select>
                    {!! $errors->first('lineadeinvestigacion_id','<span class="text-danger">:message</span>')!!}
                </div>
                <div class="col-lg-6 form-group">
                    <label for="" class="control-label">Tipo de proyecto</label>
                    <select name="tipos_proyectos_id" id="categorias" class="form-control">
                        <option value="">Tipo de proyecto</option>
                        @foreach($tipos_proyecto as $tipo)
                        <option value="{{Crypt::encrypt($tipo->id)}}">{{$tipo->nombre}}</option>
                        @endforeach
                    </select>
                    {!! $errors->first('tipos_proyectos_id','<span class="text-danger">:message</span>')!!}
                </div>
            </div>
            <div class="form-group">
                <label for="Objetivo">Objetivo</label>
                <textarea class="form-control uppercase" type="text" name="objetivo" placeholder="Objetivo"></textarea>
            ...
            <div class="row">
                <div class="col form-group">
                    ...
                    <select class="form-control" name="asesor">
                        ...
                        <option value="{{Crypt::encrypt($docente->id)}}">{{$docente->getFullName()}}</option>
                        @endforeach
                    </select>
                ...
                <div class="col form-group">
                    ...
                    <input class="form-control uppercase" type="text" name="empresa" placeholder="Empresa">
            ...
            <div class="collapse" id="integrantes">
                ...
                <h5>Integrantes</h5>
                <h6>Integrante #1: {{Auth::guard()->user()->getFullName()}}</h6>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="alumnos[]">
                        ...
                        @if (Auth::guard()->user()->id!=$alumno->id && $alumno->id_proyecto ==null &&
                        ($alumno->nombre!=null || $alumno->apellidoP!=null))
                        <!-- && $alumno->acceso==0 -->
                        <option value="{{Crypt::encrypt($alumno->id)}}">{{$alumno->getFullName()}}</option>
                        @endif
                        @endforeach
                    </select>
                </div>
